...
def data_prep():
    ...
    be_list = [['Active_Age_Days', 4], ['Account_Age', 5], ['recency', 8], ['LOGIN_CNT', 5]]
    for i in be_list:
        ft.bin_expansion(member_filter, i[0], ft.bin_generator(i[1]), source_del=True)
    ...
    """
    ggplot(aes(x='Account_Age'), data=member_attributes) + geom_histogram(binwidth=1)
